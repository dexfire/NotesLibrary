---
title: 怎么创建一个Jekyll主题？
permalink: /docs/themes/
---

1. 使用 `jekyll new-theme <theme-name>` 来初始化一个主题模板。
2. 修改主题，主要涉及几个内容：
    1. `_layouts` ：默认布局文件，页面级别的，可以是md格式的，也可以在其中使用layout
    2. `_includes` ：默认可引用组件，可以是md格式的
    3. `_sass` ：样式源文件
    4. `asserts` ：附加文件，在build过程中`asserts` **文件夹**会被拷贝到服务器根目录。
    5. `<theme-name>.gemspec` ：gem定义文件
    6. `README.md` ：文档
